...
module rob #(
    ...
    logic rob_mem_valid [DEPTH];
    logic rob_mem_done [DEPTH];
    ...
    assign commit_valid = rob_mem_valid[commit_ptr] &&
                          rob_mem_done[commit_ptr];
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (disp_valid && disp_ready) begin
                rob_mem_valid[alloc_ptr] <= 1'b1;
                rob_mem_done[alloc_ptr]  <= 1'b0;
            ...
            if (cpl_valid) begin
                rob_mem_done[cpl_tag] <= 1'b1;
            end

            // Commit (in-order)
            if (commit_valid && commit_ready) begin
                rob_mem_valid[commit_ptr] <= 1'b0;
                rob_mem_done[commit_ptr]  <= 1'b0;
